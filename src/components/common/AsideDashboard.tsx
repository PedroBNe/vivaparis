...
export default function AsideDashboard({
  ...
  return (
    <aside
      className={`fixed left-0 top-0 z-40 h-screen bg-gray-800 text-white transition-all duration-300 ease-in-out ${
        expanded ? "w-64" : "w-16"
      }`}
    >
          ...
          <nav className="space-y-2 p-2">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center rounded-lg px-3 py-2 text-sm font-medium hover:bg-gray-700 ${
                  pathname === item.href
